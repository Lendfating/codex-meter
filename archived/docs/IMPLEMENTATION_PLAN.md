...
- 账号级每日 Token 与本机每日 Token 的历史参考对账，以及同账号多机器的未观测 Token 估计。
...
11. 本机日期统一按机器配置的本地时区处理；当前默认 `Asia/Shanghai`。App Server 原始 `startDate` 原样保存，仅在派生视图中按本机日期比较。
12. `account/usage/read` 是延迟的账号 Token 参考源，启动/重连、每 6 小时和跨日后采集；当前日期缺失不等于 0。
13. Token 参考差额只用于历史核对和未观测量提示，不参与 Credit、周窗口占比或人工确认容量。
...
- 保存 `account/usage/read` 的脱敏 fixture，至少覆盖已结算日期、当前日期缺 bucket 和延迟补齐两次读取。
...
- 实现账号身份、上下文区间、token 观察、quota 快照、账号 Token 快照、Token 参考日汇总、ccusage 快照、usage delta、价格版本、容量版本、标定段和审计表。
- 时间统一存 UTC epoch 毫秒，展示时转换机器配置时区；当前默认 Asia/Shanghai。
...
- 按机器本地日期生成本机 Token 日汇总，保留事件级来源以便重算。
...
- `account/rateLimits/read` 活跃时 60 秒、空闲时 5 分钟兜底读取；reset 前后额外采样。
- `account/usage/read` 启动/重连、每 6 小时和本地日期跨天后读取；失败不阻塞本机账本，保存原始 bucket、读取时间和 freshness。
...
- 当前日期没有远程 bucket 时显示 `pending`，不写入 0；已完成日期稳定两次后才标记 `settled`。
...
- 实现 Token 参考日汇总：`account_tokens`、`local_tokens`、`unobserved_tokens`、`coverage_ratio`、`pending/stale/settled`；身份不兼容时标记 `incomparable`。
...
- Token 参考差额不改变本机 Credit 日汇总，也不进入容量候选公式。
...
- 在独立 Token 参考视图中计算账号 Token、本机 Token、未观测 Token和覆盖率；只接纳 `settled` 且可比较的日期。
...
- 日详情和趋势辅助区域：本机 Token、账号 Token、未观测 Token/覆盖率及 freshness，固定标记为非权威参考。
- 显示 Plus/Pro/Other(API)/Unknown，详情可展开 raw provider/plan 与归类来源。

门禁：

- Playwright 覆盖 Plus 单机、Pro 共享、API、跨账号日、reset、缺样和空状态。
- 覆盖 Token 参考维度：当前日远程缺 bucket、延迟补齐、Asia/Shanghai 日期边界、两台机器差额、身份不可比较。
...
- 完成 Token 参考验收：历史已结算日的账号/本机 Token 对账，当前日明确显示延迟状态。
...
10. Token 参考维度不影响 Credit 正式结果，并能标出未结算/不可比较日期。
11. 用量主页和容量标定页通过真实样本人工验收。

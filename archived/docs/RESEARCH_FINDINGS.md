...
账号级每日 Token 是独立的参考维度：本机 JSONL 用于本机明细，App Server `account/usage/read` 用于账号级每日画像。当前项目按本机配置时区对齐日期（当前为 `Asia/Shanghai`），但远程 bucket 的日期语义没有公开时区说明，因此该维度必须标为参考而不是权威结算。

本次环境验证（2026-08-04）中，账号接口最新 bucket 是 2026-08-03，当前日尚未出现；2026-08-03 的账号 Token 与本机按 `Asia/Shanghai` 切分的 JSONL Token 更接近，而 UTC/美国太平洋时区切分差异明显。该结果支持“暂按本机日期对齐”，但只是当前账号的经验样本，不是服务端时区契约。
...
这是账号侧 token 活动画像，可能跨机器，但没有模型、缓存、输出、Fast 或 Credit 拆分，不能直接用于周窗口 Credit 计算。实测和公开 issue 都表明账号级每日统计可能延迟，当前日期可能暂时没有 bucket，因此不能当作实时计数器；系统应保存原始 bucket 和读取时间，并把日期标成 `pending`、`stale` 或 `settled`。详细规则见 [Token 使用参考维度](TOKEN_USAGE_REFERENCE.md)。
...
```text
账号每日 Token = account/usage/read.dailyUsageBuckets[].tokens
本机每日 Token = JSONL 去重增量 Token
未观测 Token ≈ max(账号每日 Token - 本机每日 Token, 0)
```

在 200 美元账号同时运行于两台机器时，这个差额可以作为另一台机器或其他来源的粗略参考；它不能拆出另一台机器的精确 Token，也不能换算为 Credit。若当天混入第三方 API、账号切换、JSONL 缺读或远程 bucket 未结算，则显示“不可比较”。

采集 `account/usage/read` 不采用高频轮询：启动/重连、每 6 小时以及本地日期跨天后读取；`account/rateLimits/read` 的窗口百分比采集另行处理。
...
- [Codex App Server account usage schema and method](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md)
- [Codex Desktop profile usage statistics delay report](https://github.com/openai/codex/issues/25479)
